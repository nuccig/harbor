# Constitution — Harbor night-harbor-p2-statuschip-nav

> Reutilizada da run night-harbor-p1-tokens (aprovada pelo usuário).
> Mudanças: artifacts_dir e branching (stack sobre P1).
...
branching: feat/<slug> (esta run: feat/night-harbor-p2-statuschip-nav, stacked sobre feat/night-harbor-p1-tokens / PR #4)
...
test_expectations:
  feature: verify gate green (lint + typecheck + test); testes existentes não regridem; componente novo (StatusChip) coberto por testes de render/acessibilidade; mudanças de cor passam por review numérico de contraste (WCAG 2.1 exato) — verify gate não detecta contraste
...
artifacts_dir: .orquestrador/night-harbor-p2-statuschip-nav/
...
boundaries:
  always:
    ...
    - review numérico de contraste para qualquer par de cor novo/alterado (fundo tintado de chip incluso)
  ask_first:
