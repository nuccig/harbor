...
- Para fallback de GPU, testar `typeof` do valor real é mais verificável que testar apenas presença da propriedade com `in`; propriedades existentes mas indefinidas podem mascarar fallback.
- Em Windows sem privilégio de symlink/signing, `electron-builder --dir` pode falhar no `winCodeSign`; enquanto não há pipeline de assinatura, `win.signAndEditExecutable=false` permite smoke unsigned local.
